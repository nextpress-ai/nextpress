...
- **`createBlockDefinition` factory (blocks-report #4)** (2026-06-05): new `blocks/createBlockDefinition.tsx` — opt-in, backward-compatible factory that builds a block's `component` from a pure `render({ value, content, styles, settings, setContent, setStyles, setSettings, isPreview, isSelected, onNestedBlockChange })` fn, absorbing the `useBlockState` wiring; optional `parseContent`/`serializeContent` for structured blocks (icon). Returns a standard `BlockDefinition` (registry/renderer unchanged). Proof: spacer + divider. Build green; tests 453/453. **Decisions**: `BlockShell` (#2) NOT built (block-`${id}` already centralized in BlockRenderer; wp-block-* is intrinsic — indirection w/o dedup). `LinkSettings`/`MediaUrlField` SKIPPED (heterogeneous shapes → forced abstraction). Renderer type unification (#3) → planned as its own [Phase 8](docs/phase8-renderer-unification.md). **Rollout to pure blocks pending** (blocks whose component is only `useBlockState → renderer`; bespoke-component blocks — columns effect, post-progress scroll, post-info/post-toc queries — keep custom components).
- **`useSettingsState` hook (blocks-report #5)** (2026-06-04): new `blocks/useSettingsState.ts` encapsulates the settings-panel boilerplate (accessor resolve + force-render + `updateContent`/`updateStyles`/`updateSettings`, with `onUpdate` fallback). Unifies Pattern A (`setUpdateTrigger`) and the buggy Pattern B (stale `block.content` reads). **Proof migration**: `post-list-settings.tsx` (was Pattern B → now hook; also fixes its stale-read/no-rerender bug). Build green; tests 453/453. **Rollout COMPLETE** (2026-06-04): all 35 settings components migrated. Flat-content blocks (24, incl. post-list) use the hook fully (`content`/`updateContent`/`updateStyles`). Bespoke blocks (11: buttons/file/table/media-text/gallery structured-data; container/columns/group/image/cover/icon token/layout/media) use the hook for `accessor`+`rerender` only and keep their special update logic verbatim — behavior-identical by construction. `markdown` excluded (no settings; accessor used in its component). Delegated via frontend-engineer agents in 2 waves; verified centrally. Caught + fixed one hook bug (content resolution was merging defaults into the saved payload — `BlockEditing` tests flagged it). Build green; tests 453/453.
