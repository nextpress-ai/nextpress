...
const DEFAULT_CONTENT: ContainerContent = {
  tagName: "div",
  className: "",
};

/** Align with page design “Container width” options (`PageSettings.tsx`). */
const MAX_WIDTH_PRESETS: ReadonlyArray<{ value: string; label: string }> = [
...
const WIDTH_PRESETS: ReadonlyArray<{ value: string; label: string }> = [
...
const MIN_HEIGHT_PRESETS: ReadonlyArray<{ value: string; label: string }> = [
  { value: "__auto__", label: "Auto" },
...
const MAX_WIDTH_VALUES = new Set(MAX_WIDTH_PRESETS.map((p) => p.value));

const NUMERIC_LEN = /^(\d+(?:\.\d+)?)(px|rem)$/i;

function parseNumericLength(value: string): { num: number; unit: "px" | "rem" } | null {
...
function swapLengthUnit(value: string, target: "px" | "rem"): string | null {
...
function ContainerSettings({ block, onUpdate }: ContainerSettingsProps) {
