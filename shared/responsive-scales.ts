...
.wp-block-heading h1, h1.wp-block-heading { font-size: clamp(1.75rem, 5vw, 2.5rem); }
.wp-block-heading h2, h2.wp-block-heading { font-size: clamp(1.5rem, 4vw, 2rem); }
.wp-block-heading h3, h3.wp-block-heading { font-size: clamp(1.25rem, 3vw, 1.5rem); }
`.trim();
...
export const SPACING_BY_TIER = {
	...
	blockStackGap: { mobile: "1rem", medium: "1.25rem", large: "1.5rem" },
} as const;
